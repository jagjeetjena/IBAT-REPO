...
contract Presale is Ownable {
    ...
    uint256 public tokenRatePerEth = 1000000;
    ...
    uint256[] public lockTimePeriod = [0,30,60,90,120,150,180,210,240,270,300,330];
    uint256[] public presaleBuyingRange = [15, 30, 70,100];
    ...
    function setPresaleBuyingRange(uint256[] memory _presaleBuyingRange)
        external
        onlyOwner
    {
        presaleBuyingRange = _presaleBuyingRange;
    }

    function setRLockine(uint256[] memory _lockTimePeriod) external onlyOwner {
    ...
    function buyToken() public payable returns (address) {
        ...
        uint256 initialtokenamount = tokenAmount.mul(10).div(100);
        uint256 tenperlockamount = tokenAmount.mul(10).div(100);
        uint256 twentyperlockamountOne = tokenAmount.mul(20).div(100);
        uint256 twentyperlockamountTwo = tokenAmount.mul(20).div(100);
        uint256 twentyperlockamountThree =  tokenAmount.mul(20).div(100);
        uint256 twentyperlockamountFour = tokenAmount.mul(20).div(100);
        uint256 fourtyperlockamount = tokenAmount.mul(40).div(100);
            
        uint256 range = getBracketRange(msg.value);
        if (range == 0) {
            require(
                IToken(tokenAddress).transfer(msg.sender, initialtokenamount),
                "Insufficient balance of presale contract!"
             );

            wallet = createTokenLockedWallets(block.timestamp+ (lockTimePeriod[2] * 1 days),tenperlockamount,_msgSender());
            wallet = createTokenLockedWallets(block.timestamp+ (lockTimePeriod[4] * 1 days),twentyperlockamountOne,_msgSender());
            wallet = createTokenLockedWallets(block.timestamp+ (lockTimePeriod[6] * 1 days),twentyperlockamountTwo,_msgSender());
            wallet = createTokenLockedWallets(block.timestamp+ (lockTimePeriod[8] * 1 days),twentyperlockamountThree,_msgSender());
            wallet = createTokenLockedWallets(block.timestamp+ (lockTimePeriod[10] * 1 days),twentyperlockamountFour,_msgSender());
            
        } else {
            wallet = createTokenLockedWallets(getBracketRange(msg.value),fourtyperlockamount,_msgSender());
            wallet = createTokenLockedWallets(getBracketRange(msg.value)+ (lockTimePeriod[2] * 1 days) ,twentyperlockamountOne,_msgSender());
            wallet = createTokenLockedWallets(getBracketRange(msg.value)+ (lockTimePeriod[4] * 1 days) ,twentyperlockamountTwo,_msgSender());
            wallet = createTokenLockedWallets(getBracketRange(msg.value)+ (lockTimePeriod[6] * 1 days) ,twentyperlockamountThree,_msgSender());
        ...
        returns (uint256 range)
    {
      uint256 retrunDuration=0;
        ...
        if(amount <= presaleBuyingRange[0]*10**18){
            retrunDuration = 0;
        }
        else if(amount <= presaleBuyingRange[1]*10**18){
            retrunDuration = (block.timestamp+ (lockTimePeriod[3] * 1 days)) ;
        }
        else if(amount <= presaleBuyingRange[2]*10**18){
            retrunDuration = (block.timestamp+ (lockTimePeriod[6] * 1 days)) ;
        }
        else if(amount <= presaleBuyingRange[3]*10**18){
            retrunDuration = (block.timestamp+ (lockTimePeriod[9] * 1 days)) ;
        }
        else {
            retrunDuration = (block.timestamp+ (lockTimePeriod[12] * 1 days)) ;
        }
